...
public class GroupsManagerImpl implements GroupsManagerImplApi {
  ...
  protected final static String groupQNameSelectQuery = Compatibility.getWithClause()
		  + " temp (id,name,parent_group_id) as ( " 
  ...
  protected final static String groupMappingSelectQuery = "groups.id as groups_id, groups.parent_group_id as groups_parent_group_id, groups.name as groups_shortName, groups.dsc as groups_dsc, " +
      ...
      + "(" + groupQNameSelectQuery + "select name" + Compatibility.castToVarchar() + " from temp where temp.id = groups.id" + ") as groups_name ";
...
,new RowMapper() {
 ...
 return result;
  ...
  public Group getGroupByName(PerunSession sess, Vo vo, String name) throws GroupNotExistsException, InternalErrorException {
    try {
      return jdbc.queryForObject("select " + groupMappingSelectQuery_compat + "from groups " 
    		  	+ groupQNameJoinQuery 
    		  	+ " where qn_groups.name=? and groups.vo_id=?",
